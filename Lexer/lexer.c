...
char *
get_source_from_file (FILE *file)
{
	// Handle file error
	if (file == NULL)
	{
	...
	size_t buffer_size = fread (source, sizeof (char), source_length,
			file);
	if (buffer_size < source_length)
	{
		fprintf (stderr, "Could not read file to completion,");
...
void
initialize_lexer (const char *source)
{
...
get_next_token (bool was_newline)
{
	...
	switch (c)
	{
		...
		default:
			return spawn_token (TOKEN_ERROR);
	}
